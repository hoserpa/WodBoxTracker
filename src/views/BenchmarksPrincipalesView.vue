...
onMounted(async () => {
  try {
    loading.value = true;
    ejercicios.value = await userRmService.getByTipo("principal");
  ...
  >
    ...
    <main class="max-w-7xl mx-auto py-8 px-4">
      ...
      <div class="flex items-center gap-3 mb-8">
        ...
        ></div>
        <h2 class="text-2xl font-bold text-white">Benchmarks Principales</h2>
      </div>
      ...
      <div v-if="!loading && ejercicios.length === 0" class="text-center py-16">
        <p class="text-white/60 text-lg">No hay ejercicios principales</p>
      </div>
    </main>
  </div>
</template>
